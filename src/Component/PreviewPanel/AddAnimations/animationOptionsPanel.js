...
export default function AnimationOptionsPanel(props) {
  ...
  <div className={`animOptions ${panelMode ? '' : 'hidden'}`}>
    <h5>Animation type</h5>
    <div className="animItems">
      {animations.map((el, idx) => (
        <div
          className="animItem cursor-pointer border border-gray-300 p-2 m-1 rounded hover:bg-blue-100"
          key={el.itemName}
          data-animationname={el.animationName}
          data-animationnum={idx}
          onClick={(e) => {
            let aniN = e.target.hasAttribute('data-animationname')
              ? e.target.getAttribute('data-animationname')
              : e.target.closest('[data-animationname]').getAttribute('data-animationname');

            let subOptions = e.target.hasAttribute('data-animationnum')
              ? e.target.getAttribute('data-animationnum')
              : e.target.closest('[data-animationnum]').getAttribute('data-animationnum');
        ...
        >
          <div className="anim_img flex justify-center items-center">
            <img src={el.itemImage} />
          </div>
          <div className="animName flex justify-center items-center text-center h-10">{el.itemName}</div>
        </div>
      ))}
    </div>
  </div>

  <div className={`advOptions ${!panelMode ? '' : 'hidden'}`} draggable="false">
      ...
      <div className="anim_option_elem">
        {animParam.subAnimationList.length > 0 && (
          <>
            ...
            <div className="anim_slider_input">
              <select
                className="subAnimSelector w-full p-2 border border-gray-300 rounded"
                onChange={(e) => {
              ...
              >
                {animParam.subAnimationList.map((e) => (
                  <option key={e.addon + e.name + animParam.animationName} value={e.addon}>
                    {e.name}
                  </option>
                ))}
              </select>
        ...
        <div className="anim_slider_input flex items-center border-b border-gray-200 pb-2">
          <input
            type="range"
            min="100"
            max="4000"
            step="100"
            value={animParam.animationDuration}
          ...
          />
          <span className="ml-2 bg-blue-100 rounded px-2 py-1">
            {animParam.animationDuration < 1000 ? `${animParam.animationDuration}ms` : `${(animParam.animationDuration / 1000).toFixed(2)}s`}
          </span>
        </div>
        ...
        <div className="anim_slider_input flex items-center border-b border-gray-200 pb-2">
          <input
            ...
            max="4000"
            step="50"
            value={animParam.animationDelay}
            className="slider flex-grow"
            onChange={(e) => {
          ...
          />
          <span className="ml-2 bg-blue-100 rounded px-2 py-1">
            {animParam.animationDelay < 1000 ? `${animParam.animationDelay}ms` : `${(animParam.animationDelay / 1000).toFixed(2)}s`}
          </span>
        </div>
        ...
        <div className="anim_slider_input flex items-center border-b border-gray-200 pb-2">
          <input
            ...
            min="1"
            max="6"
            value={animParam.animationRepeat}
            className="slider flex-grow"
            onChange={(e) => {
  ...
  <div className="animFooter flex justify-between sticky bottom-0 p-3 bg-white border-t border-gray-200">
    <div className="animSettings">
      <span className="advance_options bg-blue-100 rounded-full px-4 py-1 cursor-pointer" onClick={switchMode}>
        {panelMode ? 'Advance Options' : 'Animation List'}
      </span>
    </div>
    <div className="save_animation bg-blue-600 text-white rounded-full px-4 py-1 cursor-pointer">
      <span className="apply_options" onClick={applyAnimation}>
        Apply Animation
      </span>
    </div>
  </div>
